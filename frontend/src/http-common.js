...
/*
axios_instance.interceptors.response.use(undefined, function (err) {
    ...
    store.dispatch('refreshToken') // attempt to obtain new access token by running 'refreshToken' action
      .then(access => {
        ...
        axios.request({
          ...
          console.log('Success on 2nd try')
 ...
 */
 ...
 axios_instance.defaults.headers = {
        ...
        Authorization: { toString() { return `Bearer ${store.state.access_token}`}}
    }
